...
class apiRest:
    ...
    def get_ipstr(self,location,type):
        filename = location+'\\topology.xlsx'
        data = self.read_xls_file(filename, type)
        type_list = list(data)
        ipstr = type_list[0]["HOST_IP"]
        return ipstr
    
    def get_consoleInfo(self,type):
        ...
        return console_server,console_port
    ...
    def get_arp(self,ipstr):
        url = 'https://' + ipstr + '/api/sonicos/arp/entries'
        headers = {'Content-Type': 'application/json'}
        casename = "arp entries"
        num = 0
        try:
            r = requests.get(url, headers=headers, verify=False)
            ss = r.json()
            ll = list(ss['arp']['entry'])
            print (ll)
            num = len(ll)
            if (r.status_code==200):
                print("Current {} is {}".format(casename,num))
            ...
            print "- Get{} ERROR - Web service exception, msg = {}".format(casename,e)
        finally:
            return (num)
    ...
    def post_AddressObject(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\AddressObject\\AddressObject.json"
        url = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
    ...
    def post_AddressGRP(self,ipstr,sn,location,en,step=1):
        sn,en=int(sn),int(en)
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        url_ag = 'https://' + ipstr + '/api/sonicos/address-groups/ipv4'
        json_path_ao = location + "\\JSON_FILE\\AddressGroup\\AddressObject.json"
        json_path_ag = location + "\\JSON_FILE\\AddressGroup\\AddressGroup.json"
    ...
    def post_AddressPerGRP(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        url_ag = 'https://' + ipstr + '/api/sonicos/address-groups/ipv4'
        json_path_ao = location + "\\JSON_FILE\\AddressGroup\\AddressObject.json"
    ...
    def post_AddressGrpDepth(self,ipstr,location,sn,en):
        sn,en=int(sn),int(en)
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        url_ag = 'https://' + ipstr + '/api/sonicos/address-groups/ipv4'
        json_path_ao = location + "\\JSON_FILE\\AddressGrpDepth\\AddressObject.json"
        json_path_agd0 = location + "\\JSON_FILE\\AddressGrpDepth\\AddressGrpDepth0.json"
        json_path_agd = location + "\\JSON_FILE\\AddressGrpDepth\\AddressGrpDepth.json"
    ...
    def post_ServiceObject(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\ServiceObject\\ServiceObject.json"
    ...
    def post_ServiceGroup(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_sg = 'https://' + ipstr + '/api/sonicos/service-groups'
        json_path_ag = location + "\\JSON_FILE\\ServiceGroup\\ServiceGroup.json"
        # ##############################Add service group#####################################
        num = en + 1
        for i in range(sn, num, step):
            grp = []
    ...
    def post_ServicePerGRP(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_so = 'https://' + ipstr + '/api/sonicos/service-objects'
        url_sg = 'https://' + ipstr + '/api/sonicos/service-groups'
        json_path_sg = location + "\\JSON_FILE\\ServicePerGroup\\ServicePerGroup.json"
        json_path_so = location + "\\JSON_FILE\\ServicePerGroup\\ServiceObject.json"
        # ##############################Add service object###############################
        num = en + 1
        for i in range(sn, num, step):
            grp_so = []
    ...
    def post_ServiceGrpDepth(self,ipstr,location,sn,en):
        sn,en=int(sn),int(en)
        url_sg = 'https://' + ipstr + '/api/sonicos/service-groups'
        json_path_sgd = location + "\\JSON_FILE\\ServiceGrpDepth\\ServiceGrpDepth.json"
    ...
    def post_VLAN(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\VLAN\\VLAN.json"
    ...
    def post_DHCP(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\DHCP\\DHCP.json"
    ...
    def post_ARP(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\ARP\\ARP.json"
        url = 'https://' + ipstr + '/api/sonicos/arp/entries'
        print json_path
        a, b = self.run_start_params(sn)
        c = 1
...
###############don't support post for grp vpn now###############################
    def post_GrpVPN(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\GrpVPN\\GrpVPN.json"
    ...
    def post_Schedule(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\Schedule\\Schedule.json"
    ...
    def post_Zone(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\Zone\\Zone.json"
    ...
    def post_GuestUser(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\GuestUser\\GuestUser.json"
    ...
    def post_LocalUser(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        json_path = location + "\\JSON_FILE\\LocalUser\\LocalUser.json"
    ...
    def post_vpn_pol_bymanual(self,ipstr,location,sn,en,step,grp_ao_num):
        sn,en=int(sn),int(en)
        url_vpn = 'https://' + ipstr + '/api/sonicos/vpn/policies/ipv4/site-to-site'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        url_ag = 'https://' + ipstr + '/api/sonicos/address-groups/ipv4'
        json_path_vpn = location + "\\JSON_FILE\\VPN_Policy_byManual\\VPN_Policy_byManual.json"
        json_path_ao = location + "\\JSON_FILE\\VPN_Policy_byManual\\AddressObject.json"
        json_path_ag = location + "\\JSON_FILE\\VPN_Policy_byManual\\AddressGroup.json"
        # ##############################Add address group#####################################
        num=grp_ao_num+1
        grp_ao = []
        grp = []
    ...
    def post_vpn_pol_site2site(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_vpn = 'https://' + ipstr + '/api/sonicos/vpn/policies/ipv4/site-to-site'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_vpn = location + "\\JSON_FILE\\SiteToSite\\SiteToSite.json"
        json_path_ao = location + "\\JSON_FILE\\SiteToSite\\AddressObject.json"
        a, b = self.run_start_params(sn)
        c = 2
        num = en + 1
        for i in range(sn, num, step):
    ...
    def post_vpn_pol_forPraveen(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_vpn = 'https://' + ipstr + '/api/sonicos/vpn/policies/ipv4/site-to-site'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        url_ag = 'https://' + ipstr + '/api/sonicos/address-groups/ipv4'
        json_path_vpn = location + "\\JSON_FILE\\SiteToSite_forPraveen\\SiteToSite.json"
        json_path_ao = location + "\\JSON_FILE\\SiteToSite_forPraveen\\AddressObject.json"
        json_path_ag = location + "\\JSON_FILE\\SiteToSite_forPraveen\\AddressGroup.json"
        a, b = self.run_start_params(sn)
        c = 2
        num = en + 1
        for i in range(sn, num, step):
    ...
    def post_AccessRules(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_rule = 'https://' + ipstr + '/api/sonicos/security-policies/ipv4'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_rule = location + "\\JSON_FILE\\AccessRules\\AccessRules.json"
        json_path_src = location + "\\JSON_FILE\\AccessRules\\AddressObject_src.json"
        json_path_des = location + "\\JSON_FILE\\AccessRules\\AddressObject_des.json"
        a, b = self.run_start_params(sn)
        c = 20
        num = en + 1
        for i in range(sn, num, step):
    ...
    def post_DosPolicy(self,ipstr,location,sn,en,step=1):
        sn,en,step=int(sn),int(en),int(step)
        url_dos = 'https://' + ipstr + '/api/sonicos/dos-policies'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_dos = location + "\\JSON_FILE\\DosPolicy\\DosPolicy.json"
        json_path_src = location + "\\JSON_FILE\\DosPolicy\\AddressObject_src.json"
        json_path_des = location + "\\JSON_FILE\\DosPolicy\\AddressObject_des.json"
        a, b = self.run_start_params(sn)
        c = 20
        num = en + 1
        for i in range(sn, num, step):
    ...
    def post_DosPolicy_250(self,ipstr,location,sn,en,step=1):
        sn,en,step=int(sn),int(en),int(step)
        url_dos = 'https://' + ipstr + '/api/sonicos/dos-policies'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_dos = location + "\\JSON_FILE\\DosPolicy\\DosPolicy.json"
    ...
    def post_TunnelInterface(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_TI = 'https://' + ipstr + '/api/sonicos/tunnel-interfaces/vpn'
        url_TP = 'https://' + ipstr + '/api/sonicos/vpn/policies/ipv4/tunnel-interface'
        json_path_pol = location + "\\JSON_FILE\\TunnelInterface\\VPN_Tunnel_Policy.json"
        json_path_TI =location + "\\JSON_FILE\\TunnelInterface\\TunnelInterface.json"
        a, b = self.run_start_params(sn)
        c = 2
        num = en + 1
        for i in range(sn, num, step):
    ...
    def post_NatPolicy(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_nat = 'https://' + ipstr + '/api/sonicos/nat-policies/ipv4'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_nat = location + "\\JSON_FILE\\NatPolicy\\NatPolicy.json"
        json_path_src = location + "\\JSON_FILE\\NatPolicy\\SourceAddress.json"
        json_path_ori_dst = location + "\\JSON_FILE\\NatPolicy\\ori_dst.json"
        json_path_trans_dst = location + "\\JSON_FILE\\NatPolicy\\trans_dst.json"
    ...
    def post_StaticRoute(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_rt = 'https://' + ipstr + '/api/sonicos/route-policies/ipv4'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_rt = location + "\\JSON_FILE\\StaticRoute\\StaticRoute.json"
        json_path_src = location + "\\JSON_FILE\\StaticRoute\\Src_Address.json"
        json_path_dst = location + "\\JSON_FILE\\StaticRoute\\Des_Address.json"
        json_path_gw = location + "\\JSON_FILE\\StaticRoute\\TB_Gateway.json"
    ...
    def post_StaticRouteFQDN(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_rt = 'https://' + ipstr + '/api/sonicos/route-policies/ipv4'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/fqdn'
        json_path_rt = location + "\\JSON_FILE\\StaticRouteFQDN\\StaticRouteFQDN.json"
        json_path_dst = location + "\\JSON_FILE\\StaticRouteFQDN\\DesAddressFQDN.json"
    ...
    def post_BotnetFilter(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_bot = 'https://' + ipstr + '/api/sonicos/botnet/custom-list-addresses'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_bot = location + "\\JSON_FILE\\BotnetFilter\\BotnetFilter.json"
        json_path_ao = location + "\\JSON_FILE\\BotnetFilter\\AddressObject.json"
    ...
    def post_GeoIPFilter(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_geo = 'https://' + ipstr + '/api/sonicos/geo-ip/addresses'
        url_ao = 'https://' + ipstr + '/api/sonicos/address-objects/ipv4'
        json_path_geo = location + "\\JSON_FILE\\GeoIPFilter\\GeoIPFilter.json"
        json_path_ao = location + "\\JSON_FILE\\GeoIPFilter\\AddressObject.json"
        ################################add bonet filter with variable parameter#####################################################
        a, b = self.run_start_params(sn)
        c = 2
        num = en + 1
    ...
    def post_RestAPIAgents(self,ipstr,location,sn,en,step=1):
        sn,en=int(sn),int(en)
        url_agent = 'https://' + ipstr + '/api/sonicos/user/sso/third-party-api/clients'
        json_path_agent = location + "\\JSON_FILE\\RestAPIAgents\\RestAPIAgents.json"
        # ##############################Add API Agents#####################################
        a, b = self.run_start_params(sn)
        c = 2
        num = en + 1
    ...
    def add_objects(self,type,casename,location,sn,en, step=1):
        ipstr = self.get_ipstr(location,type)
        self.api_login(ipstr)
        self.set_configMode(ipstr)
        if casename=='Address Objects':
            self.post_AddressObject(ipstr,location,sn,en,step)
        elif casename=='Address Groups':
            self.post_AddressGRP(ipstr,location,sn,en,step)
        elif casename=='AO Per Address Group':
            self.post_AddressPerGRP(ipstr,location,sn,en,step)
        elif casename=='Address Group Depth':
            self.post_AddressGrpDepth(ipstr,location,sn,en,step)
        elif casename=='Service Objects':
            self.post_ServiceObject(ipstr,location,sn,en,step)
        elif casename=='ServiceGroup':
            self.post_ServiceGroup(ipstr,location,sn,en,step)
        elif casename=='SO Per Service Group':
            self.post_ServicePerGRP(ipstr,location,sn,en,step)
        elif casename=='Service Group Depth':
            self.post_ServiceGrpDepth(ipstr,location,sn,en,step)
        elif casename=='VLANs':
            self.post_VLAN(ipstr,location,sn,en,step)
        elif casename=='DHCP Ranges':
            self.post_DHCP(ipstr,location,sn,en,step)
        elif casename=='Static ARP Entries':
            self.post_ARP(ipstr,location,sn,en,step)
        elif casename=='Schedule Objects':
            self.post_Schedule(ipstr,location,sn,en,step)
        elif casename=='Zones':
            self.post_Zone(ipstr,location,sn,en,step)
        elif casename=='Guest Users':
            self.post_GuestUser(ipstr,location,sn,en,step)
        elif casename=='Local Users':
            self.post_LocalUser(ipstr,location,sn,en,step)
        elif casename=='S2S VPN Policies':
            self.post_vpn_pol_site2site(ipstr,location,sn,en,step)
        elif casename=='Rules Per Table':
            self.post_AccessRules(ipstr,location,sn,en,step)
        elif casename=='DosPolicy':
            self.post_DosPolicy(ipstr,location,sn,en,step)
        elif casename=='Tunnel interface':
            self.post_TunnelInterface(ipstr,location,sn,en,step)
        elif casename=='NAT Policies':
            self.post_NatPolicy(ipstr,location,sn,en,step)
        elif casename=='Static Routes':
            self.post_StaticRoute(ipstr,location,sn,en,step)
        elif casename=='Static Routes for FQDN':
            self.post_StaticRouteFQDN(ipstr,location,sn,en,step)
        elif casename=='Botnet Filters':
            self.post_BotnetFilter(ipstr,location,sn,en,step)
        elif casename=='GeoIP Filters':
            self.post_GeoIPFilter(ipstr,location,sn,en,step)
        elif casename=='REST API Agents':
            self.post_RestAPIAgents(ipstr,location,sn,en,step)
        else:
            print "Please check whether the case is exist or not."

    # Commit configuration
    def api_commit(self,ipstr):
